  ...
  WORDLEJS.Wordle.prototype = {
    ...
    doLayout: function () {
      ...
      for (i = 0; i < wl; i++) {
        w = words[i];
        w.fontSize =
          ((w.weight - low) / (high - low)) *
            (this.biggestSize - this.smallestSize) +
          this.smallestSize;
        w.calculate(ctx);
      }
        ...
        for (deg = startDeg; deg < startDeg + 360; deg += dDeg) {
          ...
          if (prev == curIdx) {
            ...
            trace(
              3,
              "Mass center: " +
                parseInt(center.x, 10) +
                "x" +
                parseInt(center.y, 10)
            );
            if (this.keepCenter) {
...
(function (b) {
  function c() {}
  for (
    var d = "assert,clear,count,debug,dir,dirxml,error,exception,firebug,group,groupCollapsed,groupEnd,info,log,memoryProfile,memoryProfileEnd,profile,profileEnd,table,time,timeEnd,timeStamp,trace,warn".split(
        ","
      ),
      a;
    (a = d.pop());
